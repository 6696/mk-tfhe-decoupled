...
// b = \sum <a_i,s_i> + m + e
// m comes with a scaling factor
EXPORT void MKlweSymEncryptSingleFirst(MKLweSample* result, Torus32 message, double alpha, const MKLweKey* key){
    const int32_t n = key->LWEparams->n;
//    const int32_t parties = key->MKparams->parties;
    const int32_t i = key->MKparams->hLWE;
...
// b = \sum <a_i,s_i> + m + e
// m comes with a scaling factor
EXPORT void MKlweSymEncryptSingle(MKLweSample* result, double alpha, const MKLweKey* key){
    const int32_t n = key->LWEparams->n;
//    const int32_t parties = key->MKparams->parties;
    const int32_t i = key->MKparams->hLWE;
 ...
 * This function computes the phase of sample by using key : phi = b - \sum <a_i,s_i>
 */
EXPORT void MKlwePhaseSingle(MKLweSample* sample, const MKLweKey* key){
    const int32_t n = key->LWEparams->n;
//    const int32_t parties = key->MKparams->parties;
    const int32_t i = key->MKparams->hLWE;
...
/** encrypts a boolean using a single key */
EXPORT void MKbootsSymEncryptSingle(MKLweSample *result, const MKLweKey* key) {

    double alpha = key->MKparams->stdevLWE; //TODO: specify noise

    MKlweSymEncryptSingle(result, alpha, key);
}
/** encrypts a boolean using a single key */
EXPORT void MKbootsSymEncryptSingleFirst(MKLweSample *result, int32_t message, const MKLweKey* key) {

    Torus32 _1s8 = modSwitchToTorus32(1, 8);
    Torus32 mu = message ? _1s8 : -_1s8;
    double alpha = key->MKparams->stdevLWE; //TODO: specify noise

    MKlweSymEncryptSingleFirst(result, mu, alpha, key);
}
/** decrypts a boolean */
...
/** decrypts a boolean using a single key */
EXPORT void MKbootsSymDecryptSingle(MKLweSample *sample, const MKLweKey* key) {
    MKlwePhaseSingle(sample, key);
//    return (mu > 0 ? 1 : 0); //we have to do that because of the C binding
}
/** decrypts a boolean using a single key */
EXPORT int32_t MKbootsSymDecryptSingleFinalize(MKLweSample *sample) {
    return (sample->b > 0 ? 1 : 0); //we have to do that because of the C binding
}
