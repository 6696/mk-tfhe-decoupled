...
// key generation for every single party
EXPORT void MKLweKeyGenSingle(MKLweKey* result) {

//    const int32_t parties = result->MKparams->parties;
    const int32_t i = result->MKparams->hLWE;
...
EXPORT void MKRLweKeyGenSingle(MKRLweKey *result, MKRLweKey *PK) {
    ...
    for (int j = 0; j < dg; ++j)
    {
        ...
        // b_i = e_i + a*s_i
        torusPolynomialAddMulRFFT1(&result->Pkey[i*dg + j], result->key[i].key, &PK->Pkey[parties*dg + j]);
    }
}
// MKRLwe
// key generation for every party
// secret and public keys
EXPORT void MKRLweKeyGenPublic(MKRLweKey *PK) {
    const int32_t parties = PK->MKparams->parties;
    const int32_t dg = PK->MKparams->dg;

    // public keys
    // a
    for (int j = 0; j < dg; ++j) {
        torusPolynomialUniform(&PK->Pkey[parties * dg + j]);
    }
}
...
// TLwe
void tLweKeyCopy(TLweKey *result, TLweKey *old) {
    ...
    for (int32_t i = 0; i < k; ++i)
        for (int32_t j = 0; j < N; ++j)
            result->key[i].coefs[j] = old->key[i].coefs[j];
}

void torusPolynomialUniformCopy(TorusPolynomial *result, TorusPolynomial *old) {
    const int32_t N = result->N;

    for (int32_t i = 0; i < N; ++i)
        result->coefsT[i] = old->coefsT[i];
}

// MKRLwe
// key generation for every party
// secret and public keys
EXPORT MKRLweKey* MKRLweKeyMerge(std::vector<MKRLweKey*> PKArray, const TLweParams* RLWEparams, const MKTFHEParams* MKparams) {

    MKRLweKey* result = new_MKRLweKey(RLWEparams, MKparams);
    const int32_t parties = result->MKparams->parties;
    const int32_t dg = result->MKparams->dg;
    ...
    for (int i = 0; i < parties; ++i)
    {
        MKRLweKey* old_mkrlwe = PKArray[i];
        for (int j = 0; j < dg; ++j)
        {
            result->Pkey[i*dg + j] = old_mkrlwe->Pkey[i*dg + j];
        }
    }
...
//extractions Ring Lwe -> Lwe (extracted)
EXPORT void MKtLweExtractKeySingle(MKLweKey* LWEkey, const MKRLweKey* RLWEkey)
{
//    const int32_t parties = RLWEkey->MKparams->parties;
    const int32_t i = RLWEkey->MKparams->hLWE;
    const int32_t N = RLWEkey->MKparams->N;
...
EXPORT void MKlweCreateBootstrappingKey_v2Single(MKLweBootstrappingKey_v2* result, const MKLweKey* LWEkey,
                                           const MKRLweKey* RLWEkey, const MKLweKey* extractedLWEkey, const LweParams *extractedLWEparams,
                                           const LweParams *LWEparams, const TLweParams *RLWEparams, const MKTFHEParams* MKparams)
{
//    const int32_t parties = MKparams->parties;
    const int32_t i = MKparams->hLWE;
    const int32_t n = MKparams->n;
...
// FFT
EXPORT void init_MKLweBootstrappingKeyFFT_v2Single(MKLweBootstrappingKeyFFT_v2 *obj,
                                                   const MKLweBootstrappingKey_v2 *bk, const LweParams* LWEparams, const TLweParams* RLWEparams, const MKTFHEParams* MKparams)
{
    const int32_t n = MKparams->n;
    const int32_t dks = MKparams->dks;
...
//// FFT
//EXPORT void init_MKLweBootstrappingKeyFFT_v2Merged(MKLweBootstrappingKeyFFT_v2 *obj,
//                                                   const MKLweBootstrappingKey_v2 *bk, const LweParams* LWEparams, const TLweParams* RLWEparams, const MKTFHEParams* MKparams)
//{
//    const int32_t n = MKparams->n;
//    const int32_t dks = MKparams->dks;
